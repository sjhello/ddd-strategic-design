...
| 메뉴 구성 상품 | Menu Product      | 메뉴 구성 상품은 상품 정보와 수량으로 구성된다                     | 후라이드 콜라 콤보의 메뉴 구성 상품은 후라이드 1개와 콜라 1개.
| 메뉴 노출 상태 | Menu Displayed    | 메뉴의 노출 상태를 말한다. 메뉴의 노출 상태는 `노출됨`, `숨김`이 있다     |
...
| 메뉴 전시    | Display Menu      | 메뉴를 노출 시킨다.                                    | 
| 메뉴 숨김    | Hide Menu         | 메뉴를 숨긴다.                                       | 사장님이 직접 숨김처리 할 수 있으며, 메뉴 가격이 메뉴 구성 상품의 합보다 높다면 자동으로 숨김처리 된다.
...
| 테이블 사용 상태    | Order Table Status    | 테이블의 사용 여부를 말한다.                              |

### 주문 테이블의 행위
| 한글명          | 영문명                   | 설명                                           |예시                   |
|--------------|-----------------------|----------------------------------------------|----------------------------------------
| 테이블 사용       | Occupied              | 손님이 테이블을 사용할 수 없는 상태로 변경한다                   |테이블에 손님이 앉는다. 테이블 사용 상태가 `사용`으로 변경 된다.
| 테이블 미사용      | Not Occupied          | 손님이 테이블을 사용할 수 있는 상태로 변경한다                   |테이블에 있던 손님이 떠난다. 테이블 사용상태고 `미사용`으로 변경되며 테이블의 손님 수가 0명이 된다.
...
| 주문 타입 | Order Type      | 주문의 타입은 `배달 주문`, `포장 주문`, `매장 주문` 3가지 타입을 갖는다.                 |
| 주문 상태 | Order Status    | 주문의 상태는 `대기`, `수락`, `음식 제공됨`, `배달중`, `배달완료`, `완료` 6가지 상태를 갖는다. |
| 주문 항목 | Order Line Item | 주문의 `메뉴`                                                       |
| 주문 수량 | Order Quantity  | 주문한 메뉴의 `수량`.                                                  |`배달 주문`, `포장 주문`의 경우 음수가 될 수 없고 `매장 주문`의 경우에는 음수를 허용한다.
| 주문 가격 | Order Price     | 주문한 `메뉴의 가격(메뉴의 가격 * 수량)`                                      
| 주문서   | Order Sheet     | 주문서는 `주문 항목`, `주문 수량`, `주문 가격` 으로 이루어져 있다.                     |
...
| 매장 주문 수락     | Accept Eat In Order   | 사장님이 매장 주문을 수락한다. 주문의 상태가 `대기`에서 `수락`으로 변경된다. |
...
| 배달 장소        | Delivery Address        | 배달 주문지 라이더가 음식을 전달해야할 장소이다.                                       | 서울시 노원구....
