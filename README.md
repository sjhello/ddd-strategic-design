...
| 배달대행사 | Rider    | 키친포스에 배달 주문이 발생하는 경우 음식을 손님에게 제공해주는 라이더를 배정해주는 사용자 |
...
| 한글명      | 영문명                  | 설명                                      |
|----------|----------------------|-----------------------------------------|
| 상품       | Product              | 키친포스에서 등록할 수 있는 상품은 가격, 이름으로 구성되는 단일 품목이다 |
...
| 한글명     | 영문명               | 설명                          |
|---------|-------------------|-----------------------------|
| 메뉴그룹    | Menu Group        | 메뉴들을 특정한 기준대로 모은 `메뉴의 집합`이다 |
...
| 한글명      | 영문명               | 설명                                                  |
|----------|-------------------|-----------------------------------------------------|
| 메뉴       | Menu              | 상품을 키친포스에서 팔기 위해 등록하는 단위를 말하며 메뉴는 상품으로 구성되어 있다      |
| 메뉴 이름    | Menu name         | 메뉴의 이름을 말한다. ex> 후라이드 콜라 세트                         |
| 메뉴 가격    | Menu Price        | 메뉴 가격                                               
| 메뉴 구성 상품 | Menu Product      | 메뉴 구성 상품은 상품 정보와 수량으로 구성된다                          |
...
| 한글명          | 영문명                   | 설명                                  |
|--------------|-----------------------|-------------------------------------|
| 주문 테이블       | Order Table           | 손님들이 앉아서 식사 할 수 있는 키친포스의 테이블을 의미한다. |
| 테이블 사용 상태    | Occupied              | 테이블의 사용 여부를 말한다.                    |
| 테이블 착석       | Sit                   | 테이블이 `사용중인 상태`를 말한다.                  |
| 테이블 비어있음     | Clear                 | 테이블이 `사용중이지 않는 상태`를 말한다.              |
| 손님 수 변경      | Chage Number Of Guest | 테이블의 손님 수를 변경한다.                    |
...
| 주문서   | Order Line Item | 주문서는 `메뉴`, `수량`, `메뉴의 가격(메뉴 가격 * 수량)` 으로 이루어져 있다.        |
